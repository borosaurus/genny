...
struct SamplingLoader::PhaseConfig {
    PhaseConfig(PhaseContext& context,
        ...
        : database{(*client)[context["Database"].to<std::string>()]},
          ...
          sampleSize{context["SampleSize"].to<IntegerSpec>()} {
    }
...
void genny::actor::SamplingLoader::run() {
    ...
    for (auto&& config : _loop) {
        for (auto&& _ : config) {
            // Read the sample size documents.
            ...
            const auto sampleDocs = std::vector<bsoncxx::document::value>(cursor.begin(), cursor.end());

            if (sampleDocs.empty()) {
                BOOST_THROW_EXCEPTION(
                    InvalidConfigurationException("Collection has no documents"));
            }
            ...
            for (size_t batch = 0; batch < config->numBatches; ++batch) {
                ...
                for (size_t i = 0; i < config->insertBatchSize; ++i) {
                    docs.push_back(sampleDocs[sampleIdx].view());
...
public:
    ...
    genny::ActorVector produce(genny::ActorContext& context) {
        ...
        if (totalThreads != 1) {
            BOOST_THROW_EXCEPTION(
                InvalidConfigurationException("Only one thread supported"));
        }
        
        for (uint i = 0; i < totalThreads; ++i) {
            out.emplace_back(std::make_unique<genny::actor::SamplingLoader>(context, i, totalThreads));
        }
        return out;
    }
};
...
namespace {
std::shared_ptr<genny::ActorProducer> loaderProducer = std::make_shared<SamplingLoaderProducer>("SamplingLoader");
